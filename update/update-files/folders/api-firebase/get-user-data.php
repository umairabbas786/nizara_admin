...
if (isset($_POST['get_user_data']) && $_POST['get_user_data'] == 1) {
    ...
    */
    if (isset($_POST['user_id']) && $_POST['user_id'] != '') {
        ...
        $response = array();
        $sql_query = "SELECT * FROM `users`  WHERE status = 1 AND id=" . $id;
        $db->sql($sql_query);
        $result = $db->getResult();
    ...
    */
    if (isset($_POST['fcm_id']) && !empty($_POST['fcm_id'])) {
        ...
        if ($result == 0) {
            ...
            $response['message'] = 'FCM ID registered successfully';
        } elseif ($result == 2) {
            $response['error'] = true;
            $response['message'] = 'FCM ID already registered';
        } else {
            $response['error'] = true;
            $response['message'] = 'FCM ID not registered';
        }
    } else {
        $response["error"]   = true;
