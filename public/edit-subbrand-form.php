...
$db->select('subbrand', 'image', null, 'id=' . $ID);
$res = $db->getResult();
...
if (isset($_POST['btnEdit'])) {
	...
	if ($permissions['subcategories']['update'] == 1) {
		...
		$id = $db->escapeString($fn->xss_clean($_GET['id']));
		$sql = "SELECT slug FROM subbrand where id=" . $id;
		$db->sql($sql);
		$res = $db->getResult();
		$i = 1;
		foreach ($res as $row) {
		...
		if (!empty($name) && !empty($subtitle) && empty($error['image'])) {
			if (!empty($menu_image)) {
				...
				$sql_query = "UPDATE subbrand SET brand_id='" . $category . "', name = '" . $name . "', slug = '" . $slug . "',  subtitle = '" . $subtitle . "',image = '" . $upload_image . "'
							WHERE id =" . $ID;
				$db->sql($sql_query);
				...
				$sql_query = "UPDATE subbrand SET brand_id='" . $category . "', name = '" . $name . "', slug = '" . $slug . "', subtitle = '" . $subtitle . "', image = '" . $previous_subcategory_image . "'WHERE id = '" . $ID . "'";
				$db->sql($sql_query);
			...
			if ($update_result == 1) {
				$error['update_subcategory'] = " <section class='content-header'><span class='label label-success'>SubBrand updated Successfully</span></section>";
			} else {
				$error['update_subcategory'] = " <span class='label label-danger'>Failed update Subbrand</span>";
			}
		}
	} else {
		$error['check_permission'] = " <section class='content-header'><span class='label label-danger'>You have no permission to update subbrand</span></section>";
	}
}

$data = array();
$sql_query = "SELECT * FROM subbrand WHERE id =" . $ID;
$db->sql($sql_query);
$res_query = $db->getResult();
if (isset($_POST['btnCancel'])) { ?>
	<script>
		window.location.href = "subbrand.php";
	</script>
<?php } ?>
<section class="content-header">
	<h1>
		Edit Subbrand <small><a href='subbrand.php'> <i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to Sub Brand</a></small></h1>
	...
	<div class="row">
		<div class="col-md-6">
			<?php if ($permissions['subcategories']['update'] == 0) { ?>
				<div class="alert alert-danger topmargin-sm">You have no permission to update subbrand.</div>
			<?php } ?>
			<!-- general form elements -->
			<div class="box box-primary">
				<div class="box-header with-border">
					<h3 class="box-title">Edit Subbrand</h3>
				</div><!-- /.box-header -->
				<!-- form start -->
				<form method="post" enctype="multipart/form-data" id="edit_subcategory_form">
					<div class="box-body">
						<div class="form-group">
							<label for="exampleInputEmail1">Brand</label><?php echo isset($error['category']) ? $error['category'] : ''; ?>
							<?php
							$db->select("brand", 'id,name');
							$res = $db->getResult();
							?>
							<select class="form-control" id="category" name="category" >
								<?php foreach ($res as $row) {
									echo "<option value=" . $row['id'];
									if ($row['id'] == $res_query[0]['brand_id']) {
										echo " selected";
									}
							...
							<?php
							$db->select('subbrand', '*', null, 'id=' . $ID);
							$res = $db->getResult();
							?>

						</div>
						<div class="form-group">
							<label for="exampleInputEmail1">SubBrand Name</label>
							<div class="msg"><?php echo isset($error['name']) ? $error['name'] : ''; ?></div>
							<input type="text" class="form-control" name="name" value="<?php echo $res[0]['name']; ?>" >
						</div>
						<div class="form-group">
							<label for="exampleInputEmail1">Subbrand Subtitle</label>
