<div id="content" class="container col-md-12">
	...
	if (isset($_POST['btnDelete'])) {
		...
		$sql_query = "SELECT image FROM subbrand WHERE id =" . $ID;
		$db->sql($sql_query);
		$res = $db->getResult();
		...
		$sql_query = "DELETE FROM subbrand WHERE id =" . $ID;
		$db->sql($sql_query);
		...
		$sql_query = "SELECT image,other_images FROM products WHERE subbrand_id =" . $ID;
		$db->sql($sql_query);
		$res = $db->getResult();
		...
		$sql_query = "DELETE FROM products WHERE subbrand_id =" . $ID;
		$db->sql($sql_query);
	...
	if ($permissions['subcategories']['delete'] == 1) { ?>
		...
		<hr />
		<form method="post">
			<p>Are you sure want to delete this Subbrand?All the Products will also be Deleted.</p>
			<input type="submit" class="btn btn-primary" value="Delete" name="btnDelete" />
			<input type="submit" class="btn btn-danger" value="Cancel" name="btnNo" />
		</form>
		<div class="separator"> </div>
	<?php } else { ?>
		<div class="alert alert-danger topmargin-sm" style="margin-top: 20px;">You have no permission to delete subbrand.</div>
		<form method="post">
