...
if (isset($_POST['btnAdd'])) {
    ...
    if ($permissions['categories']['create'] == 1) {
        ...
        $slug = $function->slugify($db->escapeString($fn->xss_clean($_POST['category_name'])));
        ...
        if (!empty($category_name) && !empty($category_subtitle) && empty($error['category_image'])) {
            ...
            $sql_query = "INSERT INTO brand (name,subtitle, image,web_image,status,slug)VALUES('$category_name', '$category_subtitle', '$upload_image','',1,'$slug')";
            $db->sql($sql_query);
            $result = $db->getResult();
            if (!empty($result)) {
                $result = 0;
            ...
            if ($result == 1) {
                $error['add_category'] = " <section class='content-header'><span class='label label-success'>Brand Added Successfully</span></section>";
            } else {
...
<section class="content-header">
    <h1>Add Brand <small><a href='brand.php'> <i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to Brands</a></small></h1>
                ...
                <form method="post" enctype="multipart/form-data">
                    <div class="box-body">
                        <div class="form-group">
                            <label for="exampleInputEmail1">Brand Name</label><?php echo isset($error['category_name']) ? $error['category_name'] : ''; ?>
                            <input type="text" class="form-control" name="category_name" required>
                        </div>
                        <div class="form-group">
                            <label for="exampleInputEmail1">Brand Subtitle</label><?php echo isset($error['category_subtitle']) ? $error['category_subtitle'] : ''; ?>
