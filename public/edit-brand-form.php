	...
	if ($permissions['categories']['update'] == 1) {
		...
		if (!empty($name) && !empty($subtitle) && empty($error['image'])) {
			...
			if (!empty($menu_image)) {
				...
				$sql_query = "UPDATE brand SET name = ' $name', slug = '$slug',  subtitle = '$subtitle',image = '$upload_image' WHERE id =  $ID";
				if ($db->sql($sql_query)) {
					$db->sql($sql_query);
				...
				$sql_query = "UPDATE brand SET name = '" . $name . "', slug = '$slug', subtitle = '" . $subtitle . "', image = '" . $res[0]['image'] . "' , status = '" . $status . "'WHERE id =" . $ID;
				$db->sql($sql_query);
				$update_result = $db->getResult();
			}
			...
			// check update result
			if ($update_result == 1) {
				$error['update_category'] = " <section class='content-header'><span class='label label-success'>Brand updated Successfully</span></section>";
			} else {
...
$sql_query = "SELECT * FROM brand WHERE id =" . $ID;
$db->sql($sql_query);
$res = $db->getResult();
...
<section class="content-header">
	<h1>
		Edit Brand<small><a href='brand.php'><i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to Brand</a></small></h1>
			...
			<div class="box box-primary">
				...
				<form id="edit_category_form" method="post" enctype="multipart/form-data">
					<div class="box-body">
						...
						<div class="form-group">
							<label for="exampleInputEmail1">Brand Subtitle</label><?php echo isset($error['subtitle']) ? $error['subtitle'] : ''; ?>
