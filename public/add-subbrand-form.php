...
if (isset($_POST['btnAdd'])) {
	...
	if ($permissions['subcategories']['create'] == 1) {
		...
		if (!empty($subcategory_name) && !empty($category_subtitle) && empty($error['category_image'])) {
			...
			$sql_query = "INSERT INTO subbrand (brand_id, name, slug, subtitle, image)
			...
			if ($result == 1) {
				$error['add_category'] = " <section class='content-header'><span class='label label-success'>Sub Brand Added Successfully</span></section>";
			} else {
				$error['add_category'] = " <span class='label label-danger'>Failed add Brand</span>";
			}
		}
	} else {
		$error['check_permission'] = " <section class='content-header'><span class='label label-danger'>You have no permission to create subbrand</span></section>";
	}
}

if (isset($_POST['btnCancel'])) {
	header("location:subbrand.php");
}

?>
<section class="content-header">
	<h1>Add Sub Brand<small><a href='subbrand.php'> <i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to Sub Brand</a></small></h1>
	...
	<div class="row">
		<div class="col-md-6">
			<?php if ($permissions['subcategories']['create'] == 0) { ?>
				<div class="alert alert-danger">You have no permission to create subbrand.</div>
			<?php } ?>
			<!-- general form elements -->
			<div class="box box-primary">
				<div class="box-header with-border">
					<h3 class="box-title">Add Sub Brand</h3>
				</div><!-- /.box-header -->
				<!-- form start -->
				<form method="post" enctype="multipart/form-data">
					<div class="box-body">
						<div class="form-group">
							<label for="exampleInputEmail1">Main Brand</label><?php echo isset($error['main_category_name']) ? $error['main_category_name'] : ''; ?>
						...
						<div class="form-group">
							<label for="exampleInputEmail1">Sub Brand Name</label><?php echo isset($error['subcategory_name']) ? $error['subcategory_name'] : ''; ?>
							<input type="text" class="form-control" name="subcategory_name" required>
						</div>
						<div class="form-group">
							<label for="exampleInputEmail1">Sub Brand Subtitle</label><?php echo isset($error['category_subtitle']) ? $error['category_subtitle'] : ''; ?>
