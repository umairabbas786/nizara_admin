...
// data of 'BRANDS' table goes here
if (isset($_GET['table']) && $_GET['table'] == 'brand') {
    ...
    $sql = "SELECT COUNT(`id`) as total FROM `brand` " . $where;
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row)
        $total = $row['total'];
    ...
    foreach ($res as $row) {
        ...
        $operate = '<a href="view-subbrand.php?id=' . $row['id'] . '"><i class="fa fa-folder-open-o"></i>View Subbrand</a>';
...
// data of 'SUBBRAND' table goes here
if (isset($_GET['table']) && $_GET['table'] == 'subbrand') {
    ...
    $sql = "SELECT COUNT(*) as total FROM `subbrand` s" . $where;
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row)
        $total = $row['total'];

    $sql = "SELECT s.*,(SELECT name FROM brand c WHERE c.id=s.brand_id) as category_name FROM `subbrand` s" . $where . " ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
    $db->sql($sql);
    $res = $db->getResult();
    $bulkData = array();
    $bulkData['total'] = $total;
    ...
    foreach ($res as $row) {
        ...
        $operate = ' <a href="edit-subbrand.php?id=' . $row['id'] . '"><i class="fa fa-edit"></i>Edit</a>';
        $operate .= ' <a class="btn-xs btn-danger" href="delete-subbrand.php?id=' . $row['id'] . '"><i class="fa fa-trash-o"></i>Delete</a>';
        $tempRow['id'] = $row['id'];
        $tempRow['name'] = $row['name'];
