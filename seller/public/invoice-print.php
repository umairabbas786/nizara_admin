...
$sql_outer = "SELECT oi.price as order_item_price,oi.id as order_item_id,oi.*,u.*,v.*,p.*,o.*,u.name as uname,oi.discounted_price as dis_price,d.name as delivery_boy,o.status as order_status,oi.active_status as order_item_status,oi.seller_id as seller_id FROM `order_items` oi JOIN users u ON u.id=oi.user_id LEFT JOIN product_variant v ON oi.product_variant_id=v.id LEFT JOIN products p ON p.id=v.product_id JOIN orders o ON o.id=oi.order_id LEFT JOIN delivery_boys d ON oi.delivery_boy_id=d.id WHERE o.id=$ID AND oi.seller_id = $seller_id";

$db->sql($sql_outer);
$res_outer = $db->getResult();

$sql1 = "SELECT * FROM seller wHERE id =" . $seller_id;
$db->sql($sql1);
...
foreach ($res_outer as $row) {
    ...
    $data = array($row['product_variant_id'], $row['product_name'], $row['quantity'], $row['measurement'], $row['measurement_unit_id'], $row['discounted_price'] * $row['quantity'], $row['discount'], $row['sub_total'], $row['order_item_status'], $row['tax_id'], $final_price);
    ...
    <section class="invoice">
        ...
        <div class="row invoice-info">
            <div class="col-sm-5 invoice-col">
                ...
                <address>
                    Address : <?= $seller_res[0]['street']; ?>,<?= $seller_res[0]['state']; ?>,<?= $pincode_res[0]['city']; ?> Pincode - <?= $pincode_res[0]['pincode']; ?>
                </address>
                <address>
            ...
            <div class="col-sm-2 invoice-col">
                ...
                <address>
                    <b>Date: </b><?php echo date('d-m-Y', strtotime($res_outer[0]['date_added'])); ?>
                </address>
                <address>
            ...
            <div class="col-xs-6 col-xs-offset-6">
                <div class="table-responsive">
                    <table class="table borderless heading">
                        ...
                        <?php
                        $final_total = $order_total_price + $total_tax_amt;
